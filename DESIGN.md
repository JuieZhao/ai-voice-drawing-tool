...
对“五角星、矩形、三角形”等确定几何图形，系统使用本地动作配方直接展开为可执行 DSL，避免 LLM 偶发算错角度或走贴图路线。对“小猫、房子、花”等简笔画目标，DeepSeek 作为动作规划器，把目标拆成圆形路径、曲线、短直线、指针移动和转向等基础动作。
...
6. 增加自动构图，使复杂路径组合更美观。
7. 让 LLM 输出“绘制步骤 + 路径动作 + 指针动作”，避免硬编码模板依赖。
