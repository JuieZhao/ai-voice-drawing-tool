...
OpenAI 保留为可选实验增强，可以尝试把更复杂的自然语言拆成结构化 JSON；但三天比赛演示不依赖它完成开放物体绘制，未命中的物体会明确提示当前配方边界。
...
对“五角星、矩形、三角形、小猫、小狗、小房子”等稳定演示目标，系统使用本地动作配方直接展开为可执行 DSL，避免 LLM 偶发算错角度或走贴图路线。OpenAI 保留为复杂口令增强，任意物体自动拆笔画放入后续展望。
