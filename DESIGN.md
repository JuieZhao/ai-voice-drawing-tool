...
OpenAI 保留为可选实验增强，可以尝试把更复杂的自然语言拆成结构化 JSON；三天比赛演示中，稳定目标优先走本地高质量运笔配方，未预制物体再交给 OpenAI 尝试拆成运笔步骤。

对于三天比赛版，完整物体不追求开放世界自动规划，而是采用“本地运笔配方库”：小猫、小狗、小房子、小花、小树等演示目标会被展开成 `move_cursor`、`draw_path` 和 `turtle_turn` 等基础动作。它不是贴图模板，画布仍然会显示指针一步步绘制。
...
对“五角星、矩形、三角形、小猫、小狗、小房子”等稳定演示目标，系统使用本地动作配方直接展开为可执行 DSL，避免 LLM 偶发算错角度或部件位置。对未预制的小物体，系统优先让 OpenAI 规划路径动作，并用本地白名单过滤，失败时再回退到澄清提示。
...
2. OpenAI 输出必须经过前端 DSL 白名单过滤，只允许执行已支持动作。
...
| 任意物体自动规划 | 需要 LLM 稳定输出路径步骤、构图关系和笔画顺序，当前先用有限运笔配方保证 Demo |
| 语音澄清对话 | 需要更完整的对话状态管理 |
...
6. 增加自动构图，使复杂路径组合更美观。
7. 让 LLM 针对任意物体稳定输出“绘制步骤 + 路径动作 + 指针动作”，逐步替代有限运笔配方。
